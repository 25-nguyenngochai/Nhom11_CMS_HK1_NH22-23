jQuery(document).ready(function($) {
    'use strict';
    var this_obj = multivendor_marketplace_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.multivendor-marketplace-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message multivendor-marketplace-plugin-install');
                button.addClass('button-primary multivendor-marketplace-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.multivendor-marketplace-recommended-plugins .multivendor-marketplace-activation-note').css('display','block');
    ...
    $(document).on('click', '.multivendor-marketplace-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .multivendor-marketplace-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .multivendor-marketplace-recommended-plugins').css('display','none');
    });
